...
__all__ = ['C3k2_PConv1', 'C3k2_PConv2']
...
class CSPPC_Bottleneck(nn.Module):  # 通道部分Conv
...
class C3kPConv(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, k=3):
        ...
        c_ = int(c2 * e)  # hidden channels
        # self.m = nn.Sequential(*(RepBottleneck(c_, c_, shortcut, g, k=(k, k), e=1.0) for _ in range(n)))
        self.m = nn.Sequential(*(CSPPC_Bottleneck(c_) for _ in range(n)))


class C3k2_PConv1(C2f):
    """Faster Implementation of CSP Bottleneck with 2 convolutions."""
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            C3k(self.c, self.c, 2, shortcut, g) if c3k else CSPPC_Bottleneck(self.c) for _ in range(n)
        )
        # 解析利用MLLABlock替换Bottneck


class C3k2_PConv2(C2f):
    """Faster Implementation of CSP Bottleneck with 2 convolutions."""
