...
__all__ = ['transnext_micro', 'transnext_small', 'transnext_base', 'transnext_tiny']
...
class TransNeXt(nn.Module):
    ...
    def __init__(self, factor=0.5, depth_factor=0.5, img_size=640, pretrain_size=None, window_size=[3, 3, 3, None],
        ...
        super().__init__()
        embed_dims = tuple([int(dim * factor) for dim in embed_dims])
...
class transnext_micro(TransNeXt):
    def __init__(self, factor, depth_factor, **kwargs):
        super().__init__(factor, depth_factor, window_size=[3, 3, 3, None],
...
if __name__ == '__main__':
    model = transnext_micro(factor=0.5, depth_factor=0.5)
    model.eval()
